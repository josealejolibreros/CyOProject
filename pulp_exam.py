...
#primera prueba, debe salir 3,1,2
U=[[1, 2, 3],
[3, 2, 1],
[1, 3, 2]]
T=[1,1,1]
...
#segunda prueba, debe salir 6,4,1
U1=[[5, 15, 2, 2, 2, 4],
[2, 2, 10, 5, 1, 6],
[7, 8, 4, 4, 4, 8]]
T1=[1, 2, 3]
...
res=int(math.factorial(cantidadParcelas)/(2*math.factorial(cantidadParcelas-2)))
keysW=[]
for i in range(1,(cantidadParcelas*(cantidadParcelas-1))+1):
...
W =LpVariable.dicts('W', (keysW,["0","1"]), 0, 1, cat='Binary')
...
for i in range(1,cantidadParcelas+1):
	...
	for k in range(1,cantidadParcelas+1):
		
		if(i!=k ):
			print("i= ",str(i)," j= ",str(k))
			#my_lp_problem +=  (-M*(X[str(i)]+T[i-1]-1)) <= (lpSum(T)-T[i-1]) 
			#my_lp_problem +=  (M*(X[str(i)]+T[i-1]-1)) >= (lpSum(T)-T[i-1])
			my_lp_problem += (X[str(i)]+T[i-1]) <= X[str(k)]+(M * (1 - W[str(n)]["0"]))
			my_lp_problem += (X[str(k)]+T[k-1]) <= (X[str(i)])+(M * (1 - W[str(n)]["1"]))
...
#Que cada Xi sea menor a m
for i in range(1,cantidadParcelas+1):
	#restriction2_1=solver.Constraint(-solver.infinity(),1)
	#restriction2_2=solver.Constraint(1,solver.infinity())
	my_lp_problem +=X[str(i)] <= (m-T[i-1]+1)
...
for i in range(1,cantidadParcelas+1):
	aux3= 1*C[str(i)]["1"]
	for j in range(2,m+1):
	    aux3+= j* C[str(i)][str(j)]
	my_lp_problem += X[str(i)] == aux3
